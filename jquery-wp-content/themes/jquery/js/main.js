			...
			StripeCheckout.open({
				...
				token: function( stripeData ) {
					...
					processMembership( data )
						.done(function() {
							window.location = "http://plugins.jquery.com/join/thanks/";
						}).fail(function() {
							showError(
...
$(function() {
	...
	demoList.on( "click", "a", function( event ) {
		...
		var filename = "/" + event.target.pathname.replace( /^\//, "" ),
			parts = filename.split( "http://plugins.jquery.com/" ),
			plugin = parts[ 3 ],
	...
	if ( currentDemo ) {
		demoList.find( "a" ).filter(function() {
			return this.pathname.split( "http://plugins.jquery.com/" )[ 4 ] === (currentDemo + ".html");
		}).click();
	}
});
