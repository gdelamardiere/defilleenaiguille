...
class browser extends uploader
{
    ...
    protected $action;
    protected $thumbsDir;
    protected $thumbsMediumDir;
    protected $thumbsTypeDir;
    protected $thumbsMediumTypeDir;
    ...
    public function __construct()
    {
        ...
        $thumbsMediumDir = $this->config['uploadDir'] . "/" . $this->config['thumbsMediumDir'];
        if ((
                !is_dir($thumbsMediumDir) &&
                !@mkdir($thumbsMediumDir, $this->config['dirPerms'])
                ) ||
                !is_readable($thumbsMediumDir) ||
                !dir::isWritable($thumbsMediumDir) ||
                (
                !is_dir("$thumbsMediumDir/{$this->type}") &&
                !@mkdir("$thumbsMediumDir/{$this->type}", $this->config['dirPerms'])
                )
        )
            $this->errorMsg("Cannot access or create thumbnails folder.");

        $this->thumbsMediumDir = $thumbsMediumDir;
        $this->thumbsMediumTypeDir = "$thumbsMediumDir/{$this->type}";
    ...
    protected function act_renameDir()
    {
        ...
        if (is_dir($thumbMediumDir))
            @rename($thumbMediumDir, dirname($thumbMediumDir) . "/$newName");
    ...
    protected function act_deleteDir()
    {
        ...
        if (is_dir($thumbDir))
            dir::prune($thumbDir);
        $thumbMediumDir = "$this->thumbsMediumTypeDir/{$this->post['dir']}";
        if (is_dir($thumbMediumDir))
            dir::prune($thumbMediumDir);
        return true;
    }
    ...
    protected function act_rename()
    {
        ...
        $thumbMediumDir = "{$this->thumbsMediumTypeDir}/{$this->post['dir']}";
        $thumbMediumFile = "$thumbMediumDir/{$this->post['file']}";

        if (file_exists($thumbMediumFile))
            @rename($thumbMediumFile, "$thumbMediumDir/" . basename($newName));
    ...
    protected function act_delete()
    {
        ...
        $thumbMedium = "{$this->thumbsMediumTypeDir}/{$this->post['dir']}/{$this->post['file']}";
        if (file_exists($thumbMedium))
            @unlink($thumbMedium);
        return true;
    }
    ...
    protected function act_cp_cbd()
    {
        ...
        foreach ($this->post['files'] as $file) {
            ...
            elseif (!@copy($path, "$dir/$base"))
                ...
                $fromThumbMedium = "{$this->thumbsMediumDir}/$file";
                if (is_file($fromThumbMedium) && is_readable($fromThumbMedium)) {
                    $toThumbMedium = "{$this->thumbsMediumTypeDir}/{$this->post['dir']}";
                    if (!is_dir($toThumbMedium))
                        @mkdir($toThumbMedium, $this->config['dirPerms'], true);
                    $toThumbMedium .= "/$base";
                    @copy($fromThumbMedium, $toThumbMedium);
                }
            }
        }
        if (count($error))
            ...
            elseif (!file::isWritable($path) || !@rename($path, "$dir/$base"))
                ...
                if (is_file($fromThumbMedium) && is_readable($fromThumbMedium)) {
                    ...
                    if (!is_dir($toThumbMedium))
                        @mkdir($toThumbMedium, $this->config['dirPerms'], true);
                    $toThumb .= "/$base";
                    @rename($fromThumbMedium, $toThumbMedium);
                }
            }
        }
        if (count($error))
    ...
    protected function act_rm_cbd()
    {
        ...
        foreach ($this->post['files'] as $file) {
            ...
            elseif (!@unlink($path))
                ...
                if (is_file($thumb))
                    @unlink($thumb);
                $thumbMedium = "{$this->thumbsMediumDir}/$file";
                if (is_file($thumbMedium))
                    @unlink($thumbMedium);
            }
        }
        if (count($error))
    ...
    protected function getFiles($dir)
    {
        ...
        $thumbMediumDir = "{$this->config['uploadDir']}/{$this->config['thumbsMediumDir']}/$dir";
        ...
        foreach ($files as $file) {
            ...
            if (is_array($size) && count($size)) {
                ...
                $thumbMedium_file = "$thumbMediumDir/" . basename($file);
                if (!is_file($thumbMedium_file))
                    $this->makeThumbMedium($file, false);
                $smallThumb =
