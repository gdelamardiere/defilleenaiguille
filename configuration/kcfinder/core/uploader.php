...
/** Settings which can override default settings if exists as keys in $config['types'][$type] array
  * @var array */
    protected $typeSettings = array('disabled', 'theme', 'dirPerms', 'filePerms', 'denyZipDownload', 'maxImageWidth', 'maxImageHeight', 'thumbWidth', 'thumbHeight','thumbMediumWidth', 'thumbMediumHeight', 'jpegQuality', 'access', 'filenameChangeChars', 'dirnameChangeChars', 'denyExtensionRename', 'deniedExts');
...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    protected function makeThumbMedium($file, $overwrite=true) {
        $gd = new gd($file);
        ...
        $thumb = substr($file, strlen($this->config['uploadDir']));
        $thumb = $this->config['uploadDir'] . "/" . $this->config['thumbsMediumDir'] . "/" . $thumb;
        $thumb = path::normalize($thumb);
        $thumbDir = dirname($thumb);
        if (!is_dir($thumbDir) && !@mkdir($thumbDir, $this->config['dirPerms'], true))
            return false;
        ...
        // Images with smaller resolutions than thumbnails
        if (($gd->get_width() <= $this->config['thumbMediumWidth']) &&
            ($gd->get_height() <= $this->config['thumbMediumHeight'])
        ) {
            $browsable = array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG);
            // Drop only browsable types
            if (in_array($gd->type, $browsable))
        ...
        // Resize image
        } elseif (!$gd->resize_fit($this->config['thumbMediumWidth'], $this->config['thumbMediumHeight']))
            return false;
